    ...
    return (
        <div>
            {options && options.length > 0 && options.map((option) => (
                <Choice getValueHandler={props.getValueHandler} isStudent={props.isStudent} published={props.published} questionIndex={props.index} index={options.indexOf(option)}  onRename={renameOption} onDelete={deleteOption} length={options.length} option={option} key={option.id} value={option.text} name={"id"+props.id}/>
            ))}
